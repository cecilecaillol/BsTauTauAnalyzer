...
int main(int argc, char** argv) {
    ...
    arbre->SetBranchAddress("e1_cutbased", &e1_cutbased);
   ...
   for (Int_t i = 0; i < nentries_wtn; i++) {
	...
	TLorentzVector my_mu; my_mu.SetPtEtaPhiM(mu1_pt,mu1_eta,mu1_phi,0.105);
        TLorentzVector my_ele; my_ele.SetPtEtaPhiM(e1_pt,e1_eta,e1_phi,0);

	if (fabs(my_mu.Eta())>2.4) continue;
        if (fabs(my_ele.Eta())>2.5) continue;
	if (my_mu.DeltaR(my_ele)<0.5) continue;
	if (my_mu.Pt()<25 and my_ele.Pt()<25) continue;
	if (my_mu.Pt()<20 or my_ele.Pt()<20) continue;
	if (!pass_jvm) continue;

	// Trigger block
	bool is_singlemu = (HLT_IsoMu24 and mu1_singletrg);
	bool is_singleele = (HLT_Ele30_WPTight_Gsf and e1_singletrg);
	bool is_crosstrg = ((HLT_Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_DZ or HLT_Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL or HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL_DZ or HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL) and e1_crosstrg and mu1_crosstrg);
	if (!is_singlemu and !is_singleele and !is_crosstrg) continue;
	if (sample=="Muon" and !is_singlemu) continue;
	if (sample=="EGamma" and (is_singlemu or !is_singleele)) continue;
	if (sample=="MuonEG" and (is_singlemu or is_singleele or !is_crosstrg)) continue;

	// Block ID/iso/charge
	bool is_OS = (e1_charge*mu1_charge<0);
	...
	float aweight=1.0;
	if (name!="data_obs"){
	   aweight=aweight*genWeight*mu1_ID_sf*mu1_iso_sf*e1_ID_sf*e1_reco_sf; 
	...
	float mt1=TMass_F(my_ele.Pt(), my_ele.Px(), my_ele.Py(), my_met.Pt(), my_met.Phi());
        float mt2=TMass_F(my_mu.Pt(), my_mu.Px(), my_mu.Py(), my_met.Pt(), my_met.Phi());
