...
int main(int argc, char** argv) {
    ...
    arbre->SetBranchAddress("mu1_pt", &mu1_pt);
    arbre->SetBranchAddress("mu1_eta", &mu1_eta);
    arbre->SetBranchAddress("mu1_phi", &mu1_phi);
    arbre->SetBranchAddress("mu1_dxy", &mu1_dxy);
    arbre->SetBranchAddress("mu1_dz", &mu1_dz);
    arbre->SetBranchAddress("mu1_charge", &mu1_charge);
    arbre->SetBranchAddress("mu1_tightId", &mu1_tightId);
    arbre->SetBranchAddress("mu1_iso", &mu1_iso);
    arbre->SetBranchAddress("mu1_ID_sf", &mu1_ID_sf);
    arbre->SetBranchAddress("mu1_iso_sf", &mu1_iso_sf);
    arbre->SetBranchAddress("mu2_pt", &mu2_pt);
    arbre->SetBranchAddress("mu2_eta", &mu2_eta);
    arbre->SetBranchAddress("mu2_phi", &mu2_phi);
    arbre->SetBranchAddress("mu2_dxy", &mu2_dxy);
    arbre->SetBranchAddress("mu2_dz", &mu2_dz);
    arbre->SetBranchAddress("mu2_charge", &mu2_charge);
    arbre->SetBranchAddress("mu2_tightId", &mu2_tightId);
    arbre->SetBranchAddress("mu2_ID_sf", &mu2_ID_sf);
    arbre->SetBranchAddress("mu2_iso_sf", &mu2_iso_sf);
    ...
    arbre->SetBranchAddress("mu1_singletrg", &mu1_singletrg);
    arbre->SetBranchAddress("mu1_crosstrg", &mu1_crosstrg);
    arbre->SetBranchAddress("mu1_doubletrg", &mu1_doubletrg);
    arbre->SetBranchAddress("mu2_singletrg", &mu2_singletrg);
    arbre->SetBranchAddress("mu2_crosstrg", &mu2_crosstrg);
    arbre->SetBranchAddress("mu2_doubletrg", &mu2_doubletrg);
   ...
   for (Int_t i = 0; i < nentries_wtn; i++) {
	...
	TLorentzVector my_mu1; my_mu1.SetPtEtaPhiM(mu1_pt,mu1_eta,mu1_phi,0.105);
        TLorentzVector my_mu2; my_mu2.SetPtEtaPhiM(mu2_pt,mu2_eta,mu2_phi,0.105);

	if (fabs(my_mu1.Eta())>2.4) continue;
        if (fabs(my_mu2.Eta())>2.4) continue;
	if (my_mu1.DeltaR(my_mu2)<0.5) continue;
	if (!pass_jvm) continue;

	// Trigger block
	bool is_singlemu=(HLT_IsoMu24 and (mu1_singletrg or mu2_singletrg));
	bool is_doublemu=(HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_Mass3p8 and mu1_doubletrg and mu2_doubletrg);
	if (mu1_pt<25 and mu2_pt<25) continue;
	if (mu1_pt<20 or mu2_pt<20) continue;
	if (!is_singlemu and !is_doublemu) continue;

	// Block ID/iso/charge
	bool is_OS = (mu2_charge*mu1_charge<0);
	...
	float aweight=1.0;
	if (name!="data_obs"){
	   aweight=aweight*genWeight*mu1_ID_sf*mu1_iso_sf*mu2_ID_sf*mu2_iso_sf; 
	...
	if (my_met.Pt()<20) continue;
        ...
        float mt1=TMass_F(my_mu1.Pt(), my_mu1.Px(), my_mu1.Py(), my_met.Pt(), my_met.Phi());
        float mt2=TMass_F(my_mu2.Pt(), my_mu2.Px(), my_mu2.Py(), my_met.Pt(), my_met.Phi());
        if (mt1<50 or mt2<50) continue;

	TLorentzVector save_mu1=my_mu1;
        TLorentzVector save_mu2=my_mu2;
	   ...
	   float weight2=1.0;
           ...
           float var0 = mvis;
           float var1 = my_mu1.Pt();
           float var2 = my_mu2.Pt();
           float var3 = my_mu1.Eta();
           float var4 = my_mu2.Eta();
