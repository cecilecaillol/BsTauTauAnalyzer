...
int main(int argc, char** argv) {
    ...
    TFile *fout = new TFile(Form("~/eos/taug-2/nanoplots/mutau/after_sel/histo/histo_addSSQC_MT50%s.root",variable.c_str()),"recreate");
    ...
    int catenumber[5][2] = {{5,13},{14,14},{2,4},{1,1},{0,0}};
    ...
    for (unsigned int i=0; i < cate.size(); i++){
        ...
        if (cate[i]!="QCD"){
            for (int j = catenumber[i][0];j <= catenumber[i][1]; j++){
                cout << "j= " << j << endl;
                t.Add(("/afs/cern.ch/user/x/xuqin/eos/taug-2/nanoplots/mutau/after_sel/" + allsamplename[j] + ".root").c_str());
            }
            cout << "xixi" << endl;
        ...
        //bkgMChis[i]->SetBins(40,plotmin,plotmax);
            ...
            t.Add("/afs/cern.ch/user/x/xuqin/eos/taug-2/nanoplots/mutau/after_sel/SSregion/data*.root");
            bkgMChis[i]=new TH1F(Form("h%s",cate[i].c_str()),"",nbins,plotmin,plotmax);
        //TH1F *hMC = new TH1F(Form("h%s",cate[i].c_str()),"",40,plotmin,plotmax);
            t.Draw(Form("%s>>h%s",variable.c_str(),cate[i].c_str()),"MT_muonMET<50","goff");  
            fout->cd();
            TH1F * bkgSS=new TH1F("hSS","",nbins,plotmin,plotmax);
            TChain tSS("Events");
            for (int j =0; j< allsamplename.size()-1;j++ ){
                tSS.Add(("/afs/cern.ch/user/x/xuqin/eos/taug-2/nanoplots/mutau/after_sel/SSregion/" + allsamplename[j] + ".root").c_str());
            }
            tSS.Draw(Form("%s>>hSS",variable.c_str()),"xsweight*muidsf*tauidsf*(MT_muonMET<50)","goff"); 
            fout->cd();
    ...
    c->SaveAs(Form("Plotsmutau/addSSQCD/%s_log%d_MT50.png",variable.c_str(),log));
    fout->Close();
