...
int main(int argc, char** argv) {
    ...
    else if (sample == "ZZTo2Q2Nu") xs=4.561;
    ...
    arbre->SetBranchAddress("LepCand_tauidMsfDM",&LepCand_tauidMsfDM); //tau id scale factor
    arbre->SetBranchAddress("LepCand_taues",&LepCand_taues); //tau energy scale factor
    arbre->SetBranchAddress("LepCand_tautriggersf",&LepCand_tautriggersf);//tau trigger scale factor    
    ...
    tout->Branch("tau1idsf",&tau1idsf);
    tout->Branch("tau2idsf",&tau2idsf);

    tout->Branch("tau1tgsf",&tau1tgsf);
    tout->Branch("tau2tgsf",&tau2tgsf);

    tout->Branch("tau1es",&tau1es);
    tout->Branch("tau2es",&tau2es);
    ...
    for (Int_t i = 0; i < nentries_wtn; i++) {
        ...
        tau1idsf = LepCand_tauidMsfDM[0];
        tau2idsf = LepCand_tauidMsfDM[1];

        tau1tgsf = LepCand_tautriggersf[0];
        tau2tgsf = LepCand_tautriggersf[1];

        tau1es = LepCand_taues[0];
        tau2es = LepCand_taues[0];

        my_tau1=my_tau1*tau1es;
        my_tau2=my_tau2*tau2es;
