...
int main(int argc, char** argv) {
    ...
    int catenumber[4][2] = {{5,14},{2,4},{0,0},{1,1}};
    ...
    for (unsigned int i=0; i < allsamplename.size(); i++){
        ...
        t.SetBranchAddress("tau1tgsf",&tau1tgsf);
        t.SetBranchAddress("tau2tgsf",&tau2tgsf);
        t.SetBranchAddress("pu_weight",&pu_weight);
        t.GetEntry(i);
        ...
        t.Draw(Form("%s>>h%s",variable.c_str(),allsamplename[i].c_str()),"xsweight*tau1idsf*tau2idsf*tau1tgsf*tau2tgsf*pu_weight","goff");
    ...
    */
    ...
    for (unsigned int i=0; i < cate.size(); i++){
        ...
        t.Draw(Form("%s>>h%s",variable.c_str(),cate[i].c_str()),"xsweight*tau1idsf*tau2idsf*tau1tgsf*tau2tgsf*pu_weight","goff");  
        cout<< "xixi" << endl;   
        //bkgMChis.push_back(hMC);
    ...
    c->SaveAs(Form("Plotstautau/%s_log%d.png",variable.c_str(),log));
