...
def fit_histo_ntrk(cut, decaymode, variablename , variabletitle):
    ...
    fitfunc1 = TF1("fitfunc1", '[0]+expo(1)', -0.5, 19.5, 3)
    fitfunc2 = TF1("fitfunc2", 'pol1', 19.5, 99.5, 2)

    '''if decaymode==10 and cut =="W":
        fitfunc1 = TF1("fitfunc1", '[0]+expo(1)', -0.5, 14.5, 3)
        fitfunc2 = TF1("fitfunc2", 'pol1', 14.5, 99.5, 2)'''
    
    hFR.Fit( fitfunc1, 'R0' )
    hFR.Fit( fitfunc2, 'R+0' )
...
for cut in ["QCD","W"]:
    for decaymode in [0,1,10,11]:
        for variable in variablelist:
            ...
            if (variable==ntrk):
                fitfunc1,fitfunc2 = fit_histo_ntrk(cut,decaymode,variable.name,variable.title)
