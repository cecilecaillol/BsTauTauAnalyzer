if __name__ == "__main__":
    ...
    for j in range(0,ncat):
       ...
       dir0=fout.mkdir("et_"+str(j)+"_lNlA")

       for k in range(0,nbhist):
          postfix=postfixName[k]
          h0=fData.Get("et_"+str(j)+"_lNlA_anti/data_obs"+postfix)
          h0.Add(fVV.Get("et_"+str(j)+"_lNlA_anti/VV"+postfix),-1)
          h0.Add(fDY.Get("et_"+str(j)+"_lNlA_anti/ZLL"+postfix),-1)
          h0.Add(fDY.Get("et_"+str(j)+"_lNlA_anti/ZTT"+postfix),-1)
          h0.Add(fTT.Get("et_"+str(j)+"_lNlA_anti/TT"+postfix),-1)
          h0.Add(fST.Get("et_"+str(j)+"_lNlA_anti/ST"+postfix),-1)
          for i in range(0,h0.GetSize()-2):
              if h0.GetBinContent(i)<0:
                  h0.SetBinError(i,max(0,h0.GetBinError(i)+h0.GetBinError(i)))
                  h0.SetBinContent(i,0)
