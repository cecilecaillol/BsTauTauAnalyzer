...
int main(int argc, char** argv) {
   ...
   auto b7_1=arbre->GetBranch("nChargedPFCandidatess");
   auto b7_2=arbre->GetBranch("ChargedPFCandidates_dz");
   auto b7_5=arbre->GetBranch("ChargedPFCandidates_pt");
   ...
   for (Int_t i = 0; i < nentries_wtn; i++) {
	...
	// Block tracks
	b7_1->GetEntry(i); b7_2->GetEntry(i); b7_5->GetEntry(i);
	int ntracks=0;
	for (int nt=0; nt<nTracks; ++nt){
	   if (ChargedPFCandidates_pt[nt]>0.5 and fabs(ChargedPFCandidates_dz[nt]-0.5*LepCand_dz[0]-0.5*LepCand_dz[1])<0.05) ntracks++;
	}
	ntracks-=2;
