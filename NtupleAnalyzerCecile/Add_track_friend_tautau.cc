...
int nb_PU0=0;
   ...
   for (Int_t i = 0; i < nentries_wtn; i++) {
	...
	b1_12->GetEntry(i);b1_13->GetEntry(i);b1_14->GetEntry(i);b1_15->GetEntry(i);b1_16->GetEntry(i);b1_17->GetEntry(i);b1_18->GetEntry(i);b1_19->GetEntry(i);b1_20->GetEntry(i);b1_21->GetEntry(i);b1_22->GetEntry(i);
        ...
        for (int j=0; j<nLepCand; ++j){
           if (fabs(LepCand_eta[j])<2.1 and LepCand_id[j]==15 and j!=tau1_index and fabs(LepCand_dz[j]-LepCand_dz[tau1_index])<dz_tmp){
              tau2_index=j;
        ...
        TLorentzVector my_tau1Tk1; my_tau1Tk1.SetPtEtaPhiM(LepCand_tk1Pt[tau1_index],LepCand_tk1Eta[tau1_index],LepCand_tk1Phi[tau1_index],0);
        TLorentzVector my_tau1Tk2; my_tau1Tk2.SetPtEtaPhiM(LepCand_tk2Pt[tau1_index],LepCand_tk2Eta[tau1_index],LepCand_tk2Phi[tau1_index],0);
        TLorentzVector my_tau1Tk3; my_tau1Tk3.SetPtEtaPhiM(LepCand_tk3Pt[tau1_index],LepCand_tk3Eta[tau1_index],LepCand_tk3Phi[tau1_index],0);
        TLorentzVector my_tau2; my_tau2.SetPtEtaPhiM(LepCand_pt[tau2_index],LepCand_eta[tau2_index],LepCand_phi[tau2_index],0);
        TLorentzVector my_tau2Tk1; my_tau2Tk1.SetPtEtaPhiM(LepCand_tk1Pt[tau2_index],LepCand_tk1Eta[tau2_index],LepCand_tk1Phi[tau2_index],0);
        TLorentzVector my_tau2Tk2; my_tau2Tk2.SetPtEtaPhiM(LepCand_tk2Pt[tau2_index],LepCand_tk2Eta[tau2_index],LepCand_tk2Phi[tau2_index],0);
        TLorentzVector my_tau2Tk3; my_tau2Tk3.SetPtEtaPhiM(LepCand_tk3Pt[tau2_index],LepCand_tk3Eta[tau2_index],LepCand_tk3Phi[tau2_index],0);
	...
	for (int nt=0; nt<nTracks; ++nt){
                 float raw_dz=fabs(Track_dz[nt]-0.5*LepCand_dz[tau1_index]-0.5*LepCand_dz[tau2_index]);
		 ...
		 if (LepCand_tk1Pt[tau1_index]>0 and fabs(Track_pt[nt]-LepCand_tk1Pt[tau1_index])/LepCand_tk1Pt[tau1_index]<0.1 and my_tau1Tk1.DeltaR(my_track)<0.002) is_matchedToETau=true;
                 if (LepCand_tk2Pt[tau1_index]>0 and fabs(Track_pt[nt]-LepCand_tk2Pt[tau1_index])/LepCand_tk2Pt[tau1_index]<0.1 and my_tau1Tk2.DeltaR(my_track)<0.002) is_matchedToETau=true;
                 if (LepCand_tk3Pt[tau1_index]>0 and fabs(Track_pt[nt]-LepCand_tk3Pt[tau1_index])/LepCand_tk3Pt[tau1_index]<0.1 and my_tau1Tk3.DeltaR(my_track)<0.002) is_matchedToETau=true;
                 if (LepCand_tk1Pt[tau2_index]>0 and fabs(Track_pt[nt]-LepCand_tk1Pt[tau2_index])/LepCand_tk1Pt[tau2_index]<0.1 and my_tau2Tk1.DeltaR(my_track)<0.002) is_matchedToETau=true;
                 if (LepCand_tk2Pt[tau2_index]>0 and fabs(Track_pt[nt]-LepCand_tk2Pt[tau2_index])/LepCand_tk2Pt[tau2_index]<0.1 and my_tau2Tk2.DeltaR(my_track)<0.002) is_matchedToETau=true;
                 if (LepCand_tk3Pt[tau2_index]>0 and fabs(Track_pt[nt]-LepCand_tk3Pt[tau2_index])/LepCand_tk3Pt[tau2_index]<0.1 and my_tau2Tk3.DeltaR(my_track)<0.002) is_matchedToETau=true;
                 ...
                 if (sample!="data_obs"){
                    ...
                    float BScorrected_dz = fabs(z_corr - PV_z-0.5*LepCand_dz[tau1_index]-0.5*LepCand_dz[tau2_index]);
	...
	if (fabs(LepCand_dz[tau1_index]-LepCand_dz[tau2_index])>0.2) {ntracks=199; ntracksPU=199; ntracksHS=199; ntracksAll=199;}
