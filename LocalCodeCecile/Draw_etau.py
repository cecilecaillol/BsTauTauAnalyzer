...
if args.year=="Run2":
   file=ROOT.TFile("datacard_etau_Run2.root","r")
...
name=["mvis_nt0","mvis_nt1","mvis_nt3or4","aco_lowmvis","ntracks_lowmvis","ntracks_highmvis"]
title=["m_{vis} (GeV)","m_{vis} (GeV)","m_{vis} (GeV)","Acoplanarity","N_{tracks}","N_{tracks}"]
ncat=6
...
for i in range (0,ncat):
   ...
   #if "mvis" in name[i] and "3or4" not in name[i]: # blind
   ...
   legende.AddEntry(GGWW,"Excl. bkg.","f")
   ...
   c.Modified()
   c.SaveAs("plots_etau_"+args.year+"/control_"+name[i]+".pdf")
   c.SaveAs("plots_etau_"+args.year+"/control_"+name[i]+".png")
